...
from app.schemas import AskRequest, AskResponse, CheckRequest, CheckResponse
from app.services.catalog import get_check_subject
from app.services.rag.pipeline import answer_question, generate_risk_report
...
def ask_about_product(payload: AskRequest) -> AskResponse:
    if not mydata_user_exists(payload.user_id):
        raise HTTPException(status_code=404, detail="User not found")
    ...
    return answer_question(subject, payload.user_id, payload.question, payload.search_query)
