...
import { fetchDepositDetail } from "@/lib/api";
...
export default async function DepositDetailPage({
  ...
  const product = await fetchDepositDetail(finCoNo, finPrdtCd).catch(() => null);
  if (!product) notFound();
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
        <p className="text-sm text-muted font-data">{product.bank_name} · 예금</p>
