...
import { fetchMortgageLoanDetail } from "@/lib/api";
import { formatRate } from "@/lib/format";

export default async function MortgageLoanDetailPage({
  params,
}: {
  params: Promise<{ finCoNo: string; finPrdtCd: string }>;
}) {
  const { finCoNo, finPrdtCd } = await params;
  const product = await fetchMortgageLoanDetail(finCoNo, finPrdtCd).catch(() => null);
  if (!product) notFound();
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
        <p className="text-sm text-muted font-data">{product.bank_name} · 주택담보대출</p>
